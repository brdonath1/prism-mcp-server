...
describe("S40 C4 — prism_finalize commit-phase wall-clock deadline", () => {
  it("returns a structured deadline-exceeded response when commit hangs", async () => {
    ...
    // R18: a deadline shape never verified any doc, so the fallback renders
    // the disclosed-unknown form rather than a confident zero.
    expect(data.banner_text).toBe("PRISM | Session 1 | Handoff v2 | ?/10 docs (unverified)");
