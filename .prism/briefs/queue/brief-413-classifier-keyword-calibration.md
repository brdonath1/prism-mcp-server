# Brief 413 — Classifier keyword calibration
...
Add a test group `keyword calibration (S109 / brief-413)`:
...
Bump `SERVER_VERSION` in `src/config.ts` from `"4.2.0"` to `"4.3.0"` with a comment block describing brief-413 (classifier keyword calibration).
...
<!-- EOF: brief-413-classifier-keyword-calibration.md -->
