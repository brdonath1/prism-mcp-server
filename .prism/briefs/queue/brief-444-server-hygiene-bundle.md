...
# Brief 444 — server hygiene bundle
...
unlogged-ID warning. **Re-dispatch of brief-441** (S148-overnight; failed at the
daemon's git-fetch step before preflight — transient, never executed; content
unchanged; re-numbered 441→444, depends_on retargeted 440→443). **Depends on
brief-443** (both edit bootstrap.ts; runs after 443 merges). **CODE.** Six independent
sub-changes — implement and test each; the full suite must stay green.
...
- Synthesis input bounding (brief-445 / R3-dur).
