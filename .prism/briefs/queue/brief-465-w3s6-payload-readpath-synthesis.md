...
# Brief 465 — W3-S6: payload diet + read-path + synthesis cost + transport config hardening (prism-mcp-server)

**Status: PENDING**

**Re-issue note (S175):** This is the W3-S6 brief re-numbered to 465 (lineage: 462 → 463 → 464 → 465). Briefs 462/463/464 each dispatched once and hit a daemon-pane "Claude API / 401 Invalid authentication credentials" at launch; each 401-burned brief-id is permanently skipped by the poller (TRG-36), so re-runs MUST renumber — hence 465. **S175 ROOT CAUSE (resolved, INS-337):** this was NOT a stray API key and NOT a transient. The operator's `~/.zshrc` exported a STATIC `CLAUDE_CODE_OAUTH_TOKEN` (an sk-ant-oat01 setup-token, added in the D-258 rotation). That env var takes precedence over the macOS Keychain credential and carries no refresh token, so once the static access token expired, every login shell that sourced `.zshrc` — manual `claude` sessions AND Trigger panes (`create window with default profile`, a login+interactive shell) — sent the dead token and 401'd. Preflight passed throughout because the daemon runs under launchd, never sources `.zshrc`, and used the auto-refreshing Keychain credential. **FIX (verified S175):** removed the static export from `~/.zshrc`; panes + manual sessions now resolve the Keychain credential (auto-refreshing via its stored refresh token), and a real `claude -p 'reply OK'` returns OK on the CloudShift Max account. Step-0 below will now show `authMethod: claude.ai` (Keychain) with `CLAUDE_CODE_OAUTH_TOKEN: absent` — both normal post-fix. The work, baseline, and all constraints below are unchanged.
...
Create branch `brief/465-w3s6-payload-readpath-synthesis` off `origin/main`, commit the implementation + tests, push, and open a single PR to `main` titled `perf(server): payload diet + read-path efficiency/fidelity + synthesis cost dedup + transport config hardening (brief-465, W3-S6, D-257 wave 3)` with the evidence block above in the body. Do not push to main directly. Do not open more than one PR. CI must be green; the watcher auto-merges daemon PRs on green CI — that is the normal, expected path for this brief.

IMMEDIATELY after the PR is open — SELF-DEQUEUE (INS-324 §2): on the `briefs` branch, in ONE commit (message `chore: dequeue brief-465 (INS-324 self-dequeue)`), MOVE `.prism/briefs/queue/brief-465-w3s6-payload-readpath-synthesis.md` to `.prism/briefs/archive/brief-465-w3s6-payload-readpath-synthesis.md`. On a 409/422 push race, re-fetch and retry up to 3×. Never touch any other queue file. Record the dequeue commit SHA in the PR body. A later post_merge archive no-op or `archive_failed` event for this brief is EXPECTED (file already archived) and non-fatal.
...
<!-- EOF: brief-465-w3s6-payload-readpath-synthesis.md -->
