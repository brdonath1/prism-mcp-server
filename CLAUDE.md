...
└───────────────┬───────────────────────────────┘
                │ MCP Protocol (HTTPS)
┌───────────────▼───────────────────────────────┐
│  PRISM MCP Server (Railway) — v4.15.3         │
...
| `BOOT_INDEX_MODE` | optional | S202 boot-lean (brief-s202b T1), default flipped in S208 PR-S2c (D-278 two-phase completion): `compact` (default) ships the `session_state_manifest` only — the legacy `standing_rules_index` is omitted (≈ −20.9KB on the S208-measured prism baseline, dropping the duplicated 20,908 B index from every boot); `full` ships the legacy `standing_rules_index` PLUS the manifest (the pre-4.14.2 default). Rollback: `full` (env-only). |
...
| `BOOT_MASTHEAD` | optional | S208 MCP-6: which SINGLE graphical masthead the boot populates. `html` (default) ships `boot_masthead_html` and `boot_masthead_svg: null`; `svg` ships the reverse; `off` ships BOTH null and `banner_text` is the render surface (its fallback path is pre-built). Before 4.14.1 both rendered on every boot and the session used one — 2,721 measured bytes of dead payload. Precedence: a recognized value here wins; unset **or unrecognized** falls back to `BOOT_MASTHEAD_SVG`, so a typo can never re-enable a masthead an operator turned off. Rollback: unset (env-only). |
| `BOOT_MASTHEAD_SVG` | optional | brief-s202b T6, **superseded by `BOOT_MASTHEAD`** and kept as the compatibility alias: `off` still ships `boot_masthead_svg: null` AND `boot_masthead_html: null`, so a deployment that set it stays off across the 4.14.1 upgrade with no operator action. Any other value (or unset) means "a masthead renders", and `BOOT_MASTHEAD` decides which. |
...
This is enforced, not just written down: the kit installs `.claude/hooks/spawn-routing-guard.sh`
and wires it in `.claude/settings.json` as a `PreToolUse` hook on `Task|Agent|Workflow`. It
DENIES a Task/Agent spawn carrying no `model`, or a model outside `haiku|sonnet|opus|fable`
(`subagent_type: "fork"` is exempt — a fork inherits by design), and a `Workflow` whose script
holds an `agent()` call whose options object has no TOP-LEVEL `model:` key (a `model:` inside a
string or a nested object does not count), naming the offending line. The deny text repeats the
map, so a refused spawn tells you exactly how to re-issue it. The guard never edits anything and
always exits 0; it fails OPEN (allows, one stderr note) only when its own tooling is missing — an
