...
async function runDispatch(
  ...
  try {
    ...
    // 2. Run the Agent SDK with the caller's prompt directly. The Step-0 worker
    //    self-attestation preamble was removed in S183 (D-267): the cc_dispatch
    //    Railway worker uses a single fixed CLAUDE_CODE_OAUTH_TOKEN with no
    //    per-run account drift, so the Trigger-style attestation is unnecessary
    //    on this path and was leaking the CloudShift account email into PR bodies.
